...
function applyGuestCopy() {
  ...
  $("uploadAndPrintButton").textContent = settings.uploadButtonText;
...
async function runCountdownForPose(poseNumber) {
  poseIndicator.textContent = `Pose ${poseNumber} of ${settings.photoCount}`;
...
function makePlaceholderCanvas(label) {
  ...
  ctx.strokeRect(45, 45, out.width - 90, out.height - 90);
  ...
  ctx.fillText(label, out.width / 2, out.height / 2);
...
async function buildReceipt(photoCanvases, options = {}) {
  ...
  photos.slice(0, settings.photoCount).forEach((photo, index) => {
    ...
    ctx.drawImage(source, photoX, y, photoW, photoH);
    ctx.restore();
...
async function buildAdminPreview() {
  ...
  const receipt = await buildReceipt(placeholders, { cleanPreview: true, galleryLink: settings.galleryUrl });
...
async function handleTakePhoto() {
  ...
  showStatus("");
  lastPhotoCanvases = [];
  poseIndicator.textContent = "Get ready";

  for (let i = 1; i <= settings.photoCount; i++) {
    await runCountdownForPose(i);
    lastPhotoCanvases.push(capturePhoto());
    poseIndicator.textContent = i < settings.photoCount ? "Next pose" : "Done";
    if (i < settings.photoCount) await wait(700);
  }

  poseIndicator.textContent = "";

  lastGalleryLink = settings.galleryUrl;
  const previewReceipt = await buildReceipt(lastPhotoCanvases, { cleanPreview: true, galleryLink: lastGalleryLink });
  copyCanvasToCanvas(previewReceipt, receiptPreviewCanvas);
...
async function handleUploadAndPrint() {
  ...
  clearAutoReset();
  $("uploadAndPrintButton").disabled = true;
  ...
  const cleanPreview = await buildReceipt(lastPhotoCanvases, { cleanPreview: true, galleryLink });
  copyCanvasToCanvas(cleanPreview, receiptPreviewCanvas);

  if (settings.autoPrint) {
    await printReceipt();
  }

  $("uploadAndPrintButton").disabled = false;
  startAutoReset();
}
...
function bindEvents() {
  ...
  $("uploadAndPrintButton").addEventListener("click", handleUploadAndPrint);
